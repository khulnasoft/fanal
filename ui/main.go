...
import (
	...
	"github.com/khulnasoft/fanal/ui/internal/api"
	"github.com/khulnasoft/fanal/ui/internal/database/redis"
	...
	_ "github.com/khulnasoft/fanal/ui/docs"
)
...
func init() {
	addr := utils.GetStringFlagOrEnvParam("a", "FANAL_UI_ADDR", "0.0.0.0", "Listen Address")
	redisserver := utils.GetStringFlagOrEnvParam("r", "FANAL_UI_REDIS_URL", "localhost:6379", "Redis server address")
	redispassword := utils.GetStringFlagOrEnvParam("w", "FANAL_UI_REDIS_PASSWORD", "", "Redis server password")
	port := utils.GetIntFlagOrEnvParam("p", "FANAL_UI_PORT", 2802, "Listen Port")
	ttl := utils.GetStringFlagOrEnvParam("t", "FANAL_UI_TTL", "0s", "TTL for keys, the format is X<unit>, with unit (s, m, h, d, W, M, y)")
	version := flag.Bool("v", false, "Print version")
	dev := utils.GetBoolFlagOrEnvParam("x", "FANAL_UI_DEV", false, "Allow CORS for development")
	loglevel := utils.GetStringFlagOrEnvParam("l", "FANAL_UI_LOGLEVEL", "info", "Log Level")
	user := utils.GetStringFlagOrEnvParam("u", "FANAL_UI_USER", "admin:admin", "User in format <login>:<password>")
	disableauth := utils.GetBoolFlagOrEnvParam("d", "FANAL_UI_DISABLEAUTH", false, "Disable authentication")

	flag.Usage = func() {
		help := `Usage of Fanal-UI:
-a string
...
func main() {
	...
	utils.WriteLog("info", fmt.Sprintf("Fanal UI is listening on %v:%v", config.ListenAddress, config.ListenPort))
	...
	e.POST("/", api.AddEvent).Name = "add-event" // for compatibility with old Fanals
