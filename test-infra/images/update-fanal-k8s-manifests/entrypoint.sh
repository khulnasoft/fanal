#!/usr/bin/env bash
#
# Copyright (C) 2022 The KhulnaSoft Authors.
...
HELM_CHART_NAME="${HELM_CHART_NAME:-"fanal"}"
...
generate_deployment_files() {
    ...
    helm template --skip-tests --dry-run \
      "${chart_name}" "khulnasoft/${chart_name}" \
      | awk -vout=$1 -F": " '
        $0~/^# Source: / {
            file=out"/"$2;
            if (!(file in filemap)) {
...
create_pr() {
    ...
    branch="update/fanal-k8s-manifests-${GH_ORG}"
    ...
    body='Updating deployment files (automatically generated by helm template). Made using the [update-fanal-k8s-manifests](https://github.com/khulnasoft/fanal/test-infra/blob/main/config/jobs/update-fanal-k8s-manifests/update-fanal-k8s-manifests.yaml) ProwJob. Do not edit this PR.\n\n/kind update\n\n/area manifests'
...
# Meant to be run in the https://github.com/khulnasoft/deploy-kubernetes repository.
# $1: path of the file containing the token
main() {
    # Checks
    check_program "gpg"
