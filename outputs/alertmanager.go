...
func newAlertmanagerPayload(khulnasoftpayload types.KhulnasoftPayload, config *types.Configuration) []alertmanagerPayload {
	...
	for i, j := range khulnasoftpayload.OutputFields {
		...
		if strings.HasPrefix(i, "n_drop") {
			...
			if err == nil {
				var jj string
				if d == 0 {
					if khulnasoftpayload.Priority < types.Warning {
						khulnasoftpayload.Priority = types.Warning
					}
					jj = "0"
				} else {
					for _, threshold := range config.Alertmanager.DropEventThresholdsList {
						if d > threshold.Value {
							...
							if khulnasoftpayload.Priority < threshold.Priority {
								khulnasoftpayload.Priority = threshold.Priority
							}
							break
						}
					}
				...
				if jj == "" {
					jj = j.(string)
					if prio := types.Priority(config.Alertmanager.DropEventDefaultPriority); khulnasoftpayload.Priority < prio {
						khulnasoftpayload.Priority = prio
					}
				}
	...
	amPayload.Labels["source"] = "khulnasoft"
	amPayload.Labels["rule"] = khulnasoftpayload.Rule
	amPayload.Labels["eventsource"] = khulnasoftpayload.Source
	if khulnasoftpayload.Hostname != "" {
		amPayload.Labels[Hostname] = khulnasoftpayload.Hostname
	}
	if len(khulnasoftpayload.Tags) != 0 {
		sort.Strings(khulnasoftpayload.Tags)
		amPayload.Labels["tags"] = strings.Join(khulnasoftpayload.Tags, ",")
	}

	amPayload.Labels["priority"] = khulnasoftpayload.Priority.String()

	if val, ok := config.Alertmanager.CustomSeverityMap[khulnasoftpayload.Priority]; ok {
		amPayload.Labels["severity"] = val
	} else {
		amPayload.Labels["severity"] = defaultSeverityMap[khulnasoftpayload.Priority]
	}

	amPayload.Annotations["info"] = khulnasoftpayload.Output
	amPayload.Annotations["description"] = khulnasoftpayload.Output
	amPayload.Annotations["summary"] = khulnasoftpayload.Rule
	if config.Alertmanager.ExpiresAfter != 0 {
		amPayload.EndsAt = khulnasoftpayload.Time.Add(time.Duration(config.Alertmanager.ExpiresAfter) * time.Second)
	}
...
func (c *Client) AlertmanagerPost(khulnasoftpayload types.KhulnasoftPayload) {
	c.Stats.Alertmanager.Add(Total, 1)

	err := c.Post(newAlertmanagerPayload(khulnasoftpayload, c.Config), func(req *http.Request) {
