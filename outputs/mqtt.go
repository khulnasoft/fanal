...
func NewMQTTClient(config *types.Configuration, stats *types.Statistics, promStats *types.PromStatistics,
	...
	options.SetClientID("fanal-" + uuid.NewString()[:6])
...
// MQTTPublish .
func (c *Client) MQTTPublish(khulnasoftpayload types.KhulnasoftPayload) {
	...
	if err := c.MQTTClient.Publish(c.Config.MQTT.Topic, byte(c.Config.MQTT.QOS), c.Config.MQTT.Retained, khulnasoftpayload.String()).Error(); err != nil {
		go c.CountMetric(Outputs, 1, []string{"output:mqtt", "status:error"})
		c.Stats.MQTT.Add(Error, 1)
