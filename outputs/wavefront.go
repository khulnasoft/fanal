...
func (c *Client) WavefrontPost(khulnasoftpayload types.KhulnasoftPayload) {

	tags := make(map[string]string)
	tags["severity"] = khulnasoftpayload.Priority.String()
	tags["rule"] = khulnasoftpayload.Rule
	tags["source"] = khulnasoftpayload.Source

	if khulnasoftpayload.Hostname != "" {
		tags[Hostname] = khulnasoftpayload.Hostname
	}

	for tag, value := range khulnasoftpayload.OutputFields {
		switch v := value.(type) {
		case string:
			tags[tag] = v
		default:
	...
	if len(khulnasoftpayload.Tags) != 0 {
		tags["tags"] = strings.Join(khulnasoftpayload.Tags, ", ")
	...
	if c.WavefrontSender != nil {
		...
		if err := sender.SendMetric(c.Config.Wavefront.MetricName, 1, khulnasoftpayload.Time.UnixNano(), "khulnasoft-exporter", tags); err != nil {
			...
			utils.Log(utils.ErrorLvl, c.OutputType, fmt.Sprintf("Unable to send event %s: %s", khulnasoftpayload.Rule, err))
			return
		}
		if err := sender.Flush(); err != nil {
			...
			c.OTLPMetrics.Outputs.With(attribute.String("destination", "wavefront"),
				attribute.String("status", Error)).Inc()
			utils.Log(utils.ErrorLvl, c.OutputType, fmt.Sprintf("Unable to flush event %s: %s", khulnasoftpayload.Rule, err))
			return
		}
		...
		utils.Log(utils.InfoLvl, c.OutputType, fmt.Sprintf("Send Event %v OK", khulnasoftpayload.Rule))
	}
}
