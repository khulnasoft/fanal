...
var khulnasoftTestInput = `{"output":"This is a test from fanal","priority":"Debug","rule":"Test rule", "time":"2001-01-01T01:10:00Z","source":"syscalls","output_fields": {"proc.name":"fanal", "proc.tty": 1234}, "tags":["test","example"], "hostname":"test-host"}`
...
func TestMutualTlsPost(t *testing.T) {
	config := &types.Configuration{}
	config.MutualTLSFilesPath = "/tmp/fanaltests/client"
	config.MutualTLSClient.CertFile = "/tmp/fanaltests/client/client.crt"
	config.MutualTLSClient.KeyFile = "/tmp/fanaltests/client/client.key"
	config.MutualTLSClient.CaCertFile = "/tmp/fanaltests/client/ca.crt"
...
func certsetup(config *types.Configuration) (serverTLSConf *tls.Config, err error) {
	...
	ca := &x509.Certificate{
		...
		Subject: pkix.Name{
			...
			StreetAddress: []string{"Khulnasoft st"},
	...
	clientCert := &x509.Certificate{
		SerialNumber: big.NewInt(2019),
		Subject: pkix.Name{
			Organization:  []string{"Fanal"},
			Country:       []string{"US"},
			Province:      []string{""},
			Locality:      []string{"San Francisco"},
			StreetAddress: []string{"Fanalclient st"},
			PostalCode:    []string{"94016"},
		},
		NotBefore:    time.Now(),
		NotAfter:     time.Now().AddDate(10, 0, 0),
