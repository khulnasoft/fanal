...
func NewOCSFSecurityFinding(khulnasoftpayload types.KhulnasoftPayload) OCSFSecurityFinding {
	ocsfsf := OCSFSecurityFinding{
		...
		// Attacks: getMitreAttacke(khulnasoftpayload.Tags),
		Metadata: OCSFMetadata{
			Labels: khulnasoftpayload.Tags,
			Product: OCSFProduct{
				Name:       "Khulnasoft",
				VendorName: "Khulnasoft",
			},
			Version: schemaVersion,
		},
		RawData: khulnasoftpayload.String(),
		State:   "New",
		StateID: 1,
		Finding: OCSFFIndingDetails{
			CreatedTime: khulnasoftpayload.Time.UnixMilli(),
			Desc:        khulnasoftpayload.Output,
			Title:       khulnasoftpayload.Rule,
			Types:       []string{khulnasoftpayload.Source},
			UID:         khulnasoftpayload.UUID,
		},
		Message:     khulnasoftpayload.Rule,
		Observables: getObservables(khulnasoftpayload.Hostname, khulnasoftpayload.OutputFields),
		Timestamp:   khulnasoftpayload.Time.UnixMilli(),
		Status:      khulnasoftpayload.Priority.String(),
	}

	ocsfsf.SeverityID, ocsfsf.Severity = getAWSSecurityLakeSeverity(khulnasoftpayload.Priority)
	return ocsfsf
}
...
func (c *Client) EnqueueSecurityLake(khulnasoftpayload types.KhulnasoftPayload) {
	offset, err := c.Config.AWS.SecurityLake.Memlog.Write(c.Config.AWS.SecurityLake.Ctx, []byte(khulnasoftpayload.String()))
	if err != nil {
	...
	utils.Log(utils.InfoLvl, c.OutputType+" SecurityLake", fmt.Sprintf("Event queued (%v)", khulnasoftpayload.UUID))
...
func (c *Client) processNextBatch() error {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			...
			// https://github.com/khulnasoft/fanal/pull/390#discussion_r1081690326
			return err
		}
