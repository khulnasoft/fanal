...
func newSMTPPayload(khulnasoftpayload types.KhulnasoftPayload, config *types.Configuration) SMTPPayload {
	s := SMTPPayload{
		From:    "From: " + config.SMTP.From,
		To:      "To: " + config.SMTP.To,
		Subject: "Subject: [" + khulnasoftpayload.Priority.String() + "] " + khulnasoftpayload.Output,
	}

	s.Body = "From: " + config.SMTP.From + "\n"
	s.Body += "To: " + config.SMTP.To + "\n"
	s.Body += "Date: " + khulnasoftpayload.Time.Format(rfc2822) + "\n"
	...
	err = htmpl.Execute(&outhtml, khulnasoftpayload)
	if err != nil {
		utils.Log(utils.ErrorLvl, "SMTP", err.Error())
		return s
	}
...
func (c *Client) SendMail(khulnasoftpayload types.KhulnasoftPayload) {
	sp := newSMTPPayload(khulnasoftpayload, c.Config)
