...
const (
	clusterPolicyReportName string = "khulnasoft-cluster-policy-report"
	policyReportName        string = "khulnasoft-policy-report"
	policyReportSource      string = "Khulnasoft"
	...
	return &wgpolicy.PolicyReport{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"app.kubernetes.io/managed-by": "fanal",
			},
		},
...
func NewPolicyReportClient(config *types.Configuration, stats *types.Statistics, promStats *types.PromStatistics,
	...
	if config.PolicyReport.KhulnasoftNamespace == "" {
		dat, err := os.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/namespace")
		if err != nil {
			utils.Log(utils.ErrorLvl, "PolicyReport", fmt.Sprintf("Unable to get the Fanal's namespace, '%v' used instead", defaultNamespace))
		} else {
			defaultNamespace = string(dat)
		}
	} else {
		defaultNamespace = config.PolicyReport.KhulnasoftNamespace
	}
...
func (c *Client) UpdateOrCreatePolicyReport(khulnasoftpayload types.KhulnasoftPayload) {
	c.Stats.PolicyReport.Add(Total, 1)

	result := newResult(khulnasoftpayload)
	namespace := getNamespace(khulnasoftpayload.OutputFields)
...
func newResult(khulnasoftpayload types.KhulnasoftPayload) *wgpolicy.PolicyReportResult {
	...
	return &wgpolicy.PolicyReportResult{
		Policy:      khulnasoftpayload.Source,
		Rule:        khulnasoftpayload.Rule,
		Category:    "SI - System and Information Integrity",
		Source:      policyReportSource,
		Timestamp:   metav1.Timestamp{Seconds: int64(khulnasoftpayload.Time.Second()), Nanos: int32(khulnasoftpayload.Time.Nanosecond())}, //nolint:gosec // disable G115
		Severity:    mapSeverity(khulnasoftpayload),
		Result:      mapResult(khulnasoftpayload),
		Description: khulnasoftpayload.Output,
		Properties:  properties,
		Subjects:    getSubjects(khulnasoftpayload),
	}
}
...
func mapResult(event types.KhulnasoftPayload) wgpolicy.PolicyResult {
...
func mapSeverity(event types.KhulnasoftPayload) wgpolicy.PolicyResultSeverity {
...
func getSubjects(event types.KhulnasoftPayload) []corev1.ObjectReference {
