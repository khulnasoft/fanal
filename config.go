...
var httpOutputDefaults = map[string]map[string]any{
	"Slack": {
		...
		"Username":        "Fanal",
		"Channel":         "",
		"Icon":            "https://raw.githubusercontent.com/khulnasoft/fanal/master/imgs/fanal_color.png",
		"OutputFormat":    "all",
		"MessageFormat":   "",
		"MinimumPriority": "",
	},
	...
	"Teams": {
		"WebhookURL":      "",
		"ActivityImage":   "https://raw.githubusercontent.com/khulnasoft/fanal/master/imgs/fanal_color.png",
		"OutputFormat":    "all",
		"MinimumPriority": "",
	},
	"Webex": {
	...
	"STAN": {
		...
		"ClusterID":       "",
		"ClientID":        "",
		"SubjectTemplate": "khulnasoft.<priority>.<rule>",
	},
	"NATS": {
		"HostPort":        "",
		"SubjectTemplate": "khulnasoft.<priority>.<rule>",
...
func getConfig() *types.Configuration {
	...
	version := kingpin.Flag("version", "fanal version").Short('v').Bool()
	kingpin.Parse()
	...
	v.SetDefault("AWS.S3.Prefix", "khulnasoft")
	...
	v.SetDefault("Yandex.S3.Prefix", "khulnasoft")
	...
	// NB: Unfortunately khulnasoft events don't provide endtime, artificially set
	...
	if value, present := os.LookupEnv("ALERTMANAGER_CUSTOMSEVERITYMAP"); present {
		...
		for _, severitymatch := range severitymap {
			...
			if priority == types.Default {
				utils.Log(utils.ErrorLvl, "AlertManager", fmt.Sprintf("Priority '%v' is not a valid khulnasoft priority level", priorityString))
				continue
			} else if found {
	...
	if c.Alertmanager.DropEventThresholds != "" {
		...
		for _, threshold := range thresholds {
			...
			if p := strings.TrimSpace(values[1]); p == "" {
				utils.Log(utils.ErrorLvl, "AlertManager", fmt.Sprintf("Priority '%v' is not a valid khulnasoft priority level", p))
				continue
			}
